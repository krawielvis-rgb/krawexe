...
export function siteUrl() {
  return (process.env.URL || process.env.SITE_URL || "http://localhost:8888").replace(/\/$/, "");
}

export function redirectUri() {
  return process.env.PINTEREST_REDIRECT_URI || `${siteUrl()}/.netlify/functions/pinterest-callback`;
}

export async function getToken() {
  return (await store().get(TOKEN_KEY, { type: "json" })) as PinterestToken | null;
}

export async function saveToken(token: PinterestToken) {
  await store().setJSON(TOKEN_KEY, token);
}
...
export async function refreshTokenIfNeeded(token: PinterestToken) {
  ...
  const next: PinterestToken = {
    ...
    refresh_token_expires_at: data.refresh_token_expires_at
      ? Number(data.refresh_token_expires_at) * 1000
...
export async function generatePinCopy(topic: string) {
  ...
  const response = await fetch("https://api.deepseek.com/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You write high-quality Pinterest content. Return ONLY JSON with title, description, keywords. Never use clickbait, fake claims, or spam. Title max 100 chars. Description max 500 chars. keywords is an array of 5-10 short phrases.",
        },
        {
          role: "user",
          content: `Create one Pinterest pin for this topic: ${topic}. Brand: KRAWEXE, a free ATS resume scanner. The pin should teach one useful job-search/resume lesson and naturally encourage readers to visit the site.`,
        },
      ],
    }),
  });
  ...
  return {
    ...
    description: String(parsed.description || topic).slice(0, 500),
...
export async function publishPin() {
  ...
  const link = process.env.PINTEREST_LINK_URL || siteUrl();
  const response = await pinterestFetch("/pins", {
    method: "POST",
    body: JSON.stringify({
      board_id: boardId,
      title: copy.title,
      description: `${copy.description}${copy.keywords.length ? `\n\n${copy.keywords.map((k: string) => `#${k.replace(/[^a-zA-Z0-9]+/g, "").toLowerCase()}`).join(" ")}` : ""}`.slice(0, 500),
      link,
  ...
  await store().setJSON(`history/${new Date().toISOString()}`, {
    pin_id: data.id,
    title: copy.title,
    topic,
